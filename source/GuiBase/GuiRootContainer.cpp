...
{
    wii_log("guirootcontainer", "DrawFunc starting");
    if( !stop_requested ) {
    ...
    wii_log("guirootcontainer", "DrawFunc ending");
...
{
    wii_log("guirootcontainer", "starting run");
    gwd.Run(RunMainFunc, DrawFuncWrapper, this);
    wii_log("guirootcontainer", "ending run");
}
