...
static UInt8 _read(RomMapperF4device* rm, UInt16 ioPort)
{
    return rm->status;
}

static void _write(RomMapperF4device* rm, UInt16 ioPort, UInt8 value)
{	
	if (rm->inverted) {
...
static void getDebugInfo(RomMapperF4device* rm, DbgDevice* dbgDevice)
{
    ...
    dbgIoPortsAddPort(ioPorts, 0, 0xf4, DBG_IO_READWRITE, _read(rm, 0xe4));
}
...
int romMapperF4deviceCreate(int inverted) 
{
    ...
    ioPortRegister(0xf4, _read, _write, rm);
