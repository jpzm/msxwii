...
static UInt8 _read(RomMapperPanasonic* rm, UInt16 address) 
{
    int bank;
...
static void _write(RomMapperPanasonic* rm, UInt16 address, UInt8 value) 
{
    int region;
    int bank;
    int newBank;
                             ...
                             int sramSize, int mappedSlots) 
{
    ...
    slotRegister(slot, sslot, 0, rm->mappedSlots, _read, _read, _write, destroy, rm);
