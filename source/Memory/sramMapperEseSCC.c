...
static UInt8 _read(SramMapperEseSCC* rm, UInt16 address)
{
    int page = (address >> 13);
...
static void _write(SramMapperEseSCC* rm, UInt16 address, UInt8 value)
{
    int page = (address >> 13);
...
int sramMapperEseSCCCreate(char* filename, UInt8* buf, int size, int pSlot, int sSlot, int startPage, int hdId, int mode)
{
    ...
    slotRegister(pSlot, sSlot, startPage, 4, (SlotRead)_read, (SlotRead)peek,
                (SlotWrite)_write, (SlotEject)destroy, rm);
