...
static UInt8 _read(RomMapperKanji12* rm, UInt16 ioPort)
{
    UInt8 value = 0xff;
...
static void _write(RomMapperKanji12* rm, UInt16 ioPort, UInt8 value)
{	
	switch (ioPort & 0x0f) {
	case 7: 
...
int romMapperKanji12Create(UInt8* romData, int size) 
{
    ...
    ioPortRegisterSub(0xf7, _read, _write, rm);
