...
******************************************
*/
static UInt8 _read(MSXRs232* msxRs232, UInt16 address) 
{
...
static void _write(MSXRs232* msxRs232, UInt16 address, UInt8 value)
{
}
...
int romMapperMsxRs232Create(char* filename, UInt8* romData, int size, int slot, int sslot, int startPage)
{
    ...
    slotRegister(slot, sslot, startPage, pages, _read, peek, _write, destroy, msxRs232);
