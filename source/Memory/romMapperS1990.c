...
static UInt8 _read(RomMapperS1990* rm, UInt16 ioPort)
{
	switch (ioPort & 0x01) {
	case 0:
...
static void _write(RomMapperS1990* rm, UInt16 ioPort, UInt8 value)
{
	switch (ioPort & 0x01) {
	case 0:
...
static void getDebugInfo(RomMapperS1990* rm, DbgDevice* dbgDevice)
{
    ...
    dbgIoPortsAddPort(ioPorts, 0, 0xe4, DBG_IO_READWRITE, _read(rm, 0xe4));
    dbgIoPortsAddPort(ioPorts, 1, 0xe5, DBG_IO_READWRITE, _read(rm, 0xe5));
}
...
int romMapperS1990Create() 
{
    ...
    ioPortRegister(0xe4, _read, _write, rm);
    ioPortRegister(0xe5, _read, _write, rm);
