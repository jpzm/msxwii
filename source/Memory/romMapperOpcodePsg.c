...
static UInt8 _read(RomMapperOpcodePsg* rm, UInt16 ioPort)
{
    return ay8910ReadData(rm->ay8910, ioPort);
}

static void _write(RomMapperOpcodePsg* rm, UInt16 ioPort, UInt8 value)
{
    if ((ioPort & 3) == 0) {
...
int romMapperOpcodePsgCreate() 
{
    ...
    for (i = 0; i < 16; i += 4) {
        ioPortRegister(0x60 + i, NULL, _write, rm);
        ioPortRegister(0x61 + i, NULL, _write, rm);
        ioPortRegister(0x62 + i, _read, NULL, rm);
    }
