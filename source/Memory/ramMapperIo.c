...
static UInt8 _read(RamMapperIo* rm, UInt16 ioPort)
{
    return rm->port[ioPort & 3] | ~rm->mask;
}

static void _write(RamMapperIo* rm, UInt16 ioPort, UInt8 value)
{
    ioPort &= 3;
...
static void getDebugInfo(RamMapperIo* rm, DbgDevice* dbgDevice)
{
    ...
    for (i = 0; i < 4; i++) {
        dbgIoPortsAddPort(ioPorts, i, 0xfc + i, DBG_IO_READWRITE, _read(rm, 0xfc + i));
    }
}
...
int ramMapperIoCreate() 
{
    ...
    ioPortRegister(0xfc, _read, _write, rm);
    ioPortRegister(0xfd, _read, _write, rm);
    ioPortRegister(0xfe, _read, _write, rm);
    ioPortRegister(0xff, _read, _write, rm);
