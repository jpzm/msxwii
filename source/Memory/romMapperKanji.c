...
static UInt8 _read(RomMapperKanji* rm, UInt16 ioPort)
{
    UInt32* reg = &rm->address[(ioPort & 2) >> 1];
    UInt8 value;
...
static void _write(RomMapperKanji* rm, UInt16 ioPort, UInt8 value)
{	
    switch (ioPort & 0x03) {
	case 0:
...
int romMapperKanjiCreate(UInt8* romData, int size) 
{
    ...
    ioPortRegister(0xd8, NULL, _write, rm);
    ioPortRegister(0xd9, _read, _write, rm);
    ioPortRegister(0xda, NULL, _write, rm);
    ioPortRegister(0xdb, _read, _write, rm);
