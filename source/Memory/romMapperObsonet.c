...
static UInt8 _read(RomMapperObsonet* rm, UInt16 address) 
{
...
static UInt8 peek(RomMapperObsonet* rm, UInt16 address) 
{
    ...
    return _read(rm, address);
}

static void _write(RomMapperObsonet* rm, UInt16 address, UInt8 value) 
{
    if ((address & 0x3fe0) == 0x3fe0) {
        if (rm->regBank < 3) {
//        printf("W %d: %.4x  %.2x\n", rm->regBank, address & 0x1f, value);
                           ...
                           int size, int slot, int sslot, int startPage) 
{
    ...
    slotRegister(slot, sslot, startPage, 4, _read, peek, _write, destroy, rm);
