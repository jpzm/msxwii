...
static UInt8 _read(SramMapperMatsushita* rm, UInt16 ioPort)
{
	UInt8 result;
	switch (ioPort & 0x0f) {
	case 0:
...
static void _write(SramMapperMatsushita* rm, UInt16 ioPort, UInt8 value)
{
	switch (ioPort & 0x0f) {
    case 1:
        rm->cpu15 = (value & 1) == 1;
...
int sramMapperMatsushitaCreate() 
{
    ...
    ioPortRegisterSub(0x08, _read, _write, rm);
