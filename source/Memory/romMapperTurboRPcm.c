...
static UInt8 _read(RomMapperTurboRPcm* rm, UInt16 ioPort)
{
    switch (ioPort & 0x01) {
	case 0: 
...
static void _write(RomMapperTurboRPcm* rm, UInt16 ioPort, UInt8 value)
{
	switch (ioPort & 0x01) {
	case 0:
        getTimerCounter(rm);
...
int romMapperTurboRPcmCreate() 
{
    ...
    ioPortRegister(0xa4, _read, _write, rm);
    ioPortRegister(0xa5, _read, _write, rm);
