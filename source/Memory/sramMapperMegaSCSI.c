...
static UInt8 _read(SramMapperMegaSCSI* rm, UInt16 address)
{
    int page = (address >> 13);
...
static void _write(SramMapperMegaSCSI* rm, UInt16 address, UInt8 value)
{
    int page = (address >> 13);
...
int sramMapperMegaSCSICreate(char* filename, UInt8* buf, int size, int pSlot, int sSlot, int startPage, int hdId, int flag)
{
    ...
    if (rm->type) {
        slotRegister(pSlot, sSlot, startPage, 4,
                    (SlotRead)_read, (SlotRead)peek, (SlotWrite)_write,
                    (SlotEject)destroy, rm);
    } else {
        slotRegister(pSlot, sSlot, startPage, 4, NULL, NULL, (SlotWrite)_write,
                    (SlotEject)destroy, rm);
    }
