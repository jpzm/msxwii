...
static UInt8 _read(RomMapperSonyHbiV1* rm, UInt16 address)
{
    UInt8 value = 0xff;
...
static void _write(RomMapperSonyHbiV1* rm, UInt16 address, UInt8 value) 
{
    if (address >= 0x8000) {
        return;
    }
                             ...
                             int slot, int sslot, int startPage) 
{
    ...
    slotRegister(slot, sslot, startPage, 4, _read, _read, _write, destroy, rm);
