...
static UInt8 _read(RomMapperMicrosolVmx80* rm, UInt16 address)
{
    if (address == 0x3001) {
...
static void _write(RomMapperMicrosolVmx80* rm, UInt16 address, UInt8 value) 
{
                                 ...
                                 void* charRom, int charSize) 
{
    ...
    slotRegister(slot, sslot, startPage, 2, _read, _read, _write, destroy, rm);
