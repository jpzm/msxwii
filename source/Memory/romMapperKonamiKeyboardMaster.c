...
static UInt8 _read(RomMapperKonamiKeyboardMaster* rm, UInt16 ioPort)
{
    switch (ioPort) {
    case 0x00:
...
static void _write(RomMapperKonamiKeyboardMaster* rm, UInt16 ioPort, UInt8 value)
{	
    switch (ioPort) {
    case 0x00:
...
static void getDebugInfo(RomMapperKonamiKeyboardMaster* rm, DbgDevice* dbgDevice)
{
    ...
    dbgIoPortsAddPort(ioPorts, 1, 0x20, DBG_IO_READWRITE, _read(rm, 0x20));
}
                                       ...
                                       void* voiceRom, int voiceSize) 
{
    ...
    ioPortRegister(0x00, _read, _write, rm);
    ioPortRegister(0x20, _read, _write, rm);
