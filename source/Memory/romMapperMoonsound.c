...
static UInt8 _read(RomMapperMoonsound* rm, UInt16 ioPort)
{
...
static void _write(RomMapperMoonsound* rm, UInt16 ioPort, UInt8 data)
{
...
int romMapperMoonsoundCreate(char* filename, UInt8* romData, int size, int sramSize)
{
    ...
    if (boardGetMoonsoundEnable()) {
        ...
        ioPortRegister(0x7e, _read, _write, rm);
        ioPortRegister(0x7f, _read, _write, rm);
        ioPortRegister(0xc4, _read, _write, rm);
        ioPortRegister(0xc5, _read, _write, rm);
        ioPortRegister(0xc6, _read, _write, rm);
        ioPortRegister(0xc7, _read, _write, rm);
    }
    else {
