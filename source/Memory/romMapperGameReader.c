...
static UInt8 _read(RomMapperGameReader* rm, UInt16 address) 
{
...
//#define WRITE_CHECK
static void _write(RomMapperGameReader* rm, UInt16 address, UInt8 value) 
{
#ifdef WRITE_CHECK
...
//    f = fopen("c:\\grlog.txt", "w+");
    ...
    if (rm->gameReader != NULL) {
        ...
        slotRegister(slot, sslot, 0, 8, _read, _read, _write, destroy, rm);
        for (i = 0; i < 8; i++) {   
