    ...
    if (glob_dirs) {
        for (int i = 0; i < glob_dirs->count; i++) {
            ...
            if( stat(path, &s) >= 0 ) {
                ...
                for(k = first_index; k < num_files && strcasecmp(filename, items[k]->txt_col_a) > 0; k++);
    ...
    do {
        ...
        if( Run() ) {
            ...
            if( items[sel]->is_dir ) {
                ...
                char *cur_dir = strdup(directory.CreatePath(""));
                ...
                // Ask confirmation
                GuiEffectFade gEF1(GFSEL_FADE_FRAMES);
		GuiEffectFade gEF2(GFSEL_FADE_FRAMES);
                    ...
                    : GuiDialog(parent, name)
{
    ...
    // Prepare directory
    org_dir = strdup(directory.CreatePath(""));
    directory.ChangeDirectory(dir);
    root_dir = strdup(directory.CreatePath(""));
