        ...
        switch( key ) {
            ...
            case BTN_SPACE:
                ...
                if( elm == grButtonAdd ) {
                    ...
                    if( filesel->Create() ) {
                        GuiEffectFade gFE1 = GuiEffectFade(10);
                        GuiEffectFade gFE2 = GuiEffectFade(10);
                        parent->AddTop(filesel, gFE1);
                        (void)filesel->DoModal();
                        parent->Remove(filesel, gFE2);
                    }
                    Delete(filesel);
                ...
                if( elm == grButtonSettings ) {
                    ...
                    GuiEffectFade gFE1 = GuiEffectFade(10);
                    GuiEffectFade gFE2 = GuiEffectFade(10);
                    parent->AddTop(input, gFE1);
                    (void)input->DoModal();
                    parent->RemoveAndDelete(input, gFE2);
                    parent->update_screenshots = true;
                    handled = true;
                    break;
                }
...
{
    wii_log("guidlggameselect", "constructor starting");
    background = bgr;
    ...
    wii_log("guidlggameselect", "constructor ending");
}
...
bool GuiDlgGameSelect::Load(const char *dir, const char *filename, GameElement *select)
{
    ...
    wii_log("guidlggameselect", "Load ending");
    return true;
}
