...
static UInt8 _read(JoystickIO* joyIO, UInt16 address) 
{
...
static UInt8 poll(JoystickIO* joyIO, UInt16 address) 
{
    UInt8 rv;
    isPolling = 1;
    rv = _read(joyIO, address);
    isPolling = 0;
    return rv;
}

static void _write(JoystickIO* joyIO, UInt16 address, UInt8 value) 
{
    UInt32 systemTime = boardSystemTime();
...
JoystickIO* joystickIoCreate(AY8910* ay8910)
{
    ...
    ay8910SetIoPort(ay8910, _read, poll, _write, joyIO);
