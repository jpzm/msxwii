...
void setDefaultPaths(const char* rootDir)
{
    ...
    wii_log_str("setDefaultPaths", "creating", buffer);
...
void archDiskQuickChangeNotify(int driveId, char* fileName, const char* fileInZipFile)
{
    ...
    // Show popup
    GuiEffectFade gEF1(10, 0, true);
    GuiEffectFade gEF2(50, 10, true);
...
{
    wii_log("guimain", "starting constructor");
    // Resources
    GuiImage *image;
    GuiRect rect;
    GuiFontInit();
    ...
    wii_log("guimain", "ending constructor");
}
...
{
    wii_log("guimain", "bluemsxinit starting");
    int width;
    int height;

    wii_log("guimain", "bluemsxinit set paths");
    setDefaultPaths(archGetCurrentDirectory());

    properties = propCreate(resetProperties, 0, P_KBD_EUROPEAN, 0, "");

    if (resetProperties == 2) {
    	wii_log("guimain", "bluemsxinit reset properties");
        propDestroy(properties);
        return;
    }

    // Load tools
    wii_log("guimain", "bluemsxinit load tools");
    toolLoadAll(properties->language, this);

    wii_log("guimain", "bluemsxinit create video");
    video = videoCreate();
    wii_log("guimain", "bluemsxinit video created");

    if (properties->video.windowSize == P_VIDEO_SIZEFULLSCREEN) {
        wii_log("guimain", "bluemsxinit P_VIDEO_SIZEFULLSCREEN");
        ...
        if (properties->video.windowSize == P_VIDEO_SIZEX1) {
    	    wii_log("guimain", "bluemsxinit P_VIDEO_SIZEX1");
            zoom = 1;
        }
        else {
    	    wii_log("guimain", "bluemsxinit P_VIDEO_SIZEX2");
            zoom = 2;
        }
        bitDepth = 16;
    }
    ...
    wii_log("guimain", "bluemsxinit create mixer");
    mixer = mixerCreate();
    ...
    wii_log("guimain", "bluemsxinit ending");
}
...
void GuiMain::blueMsxRun(GameElement *game, char * game_dir)
{
    ...
    GuiEffectFade gEF2(20);
    RemoveAndDelete(emuSpr, gEF2);
    RemoveAndDelete(osk);
...
{
    wii_log("guimain", "Main starting");
    // Background
    wii_log("guimain", "loading background");
    background = new GuiElmBackground(this, "background");
    AddTop(background, effectDefault);

#ifdef WII
    // Init storage access
    wii_log("guimain", "checking storage");
    if( !g_bSDMounted && !g_bUSBMounted ) {
        wii_log("guimain", "no storage");
        // Prepare messagebox
    ...
    {
#endif
        wii_log("guimain", "storage mounted");
        // Please wait...
        GuiDlgMessageBox *msgbox = new GuiDlgMessageBox(this, "pleasewait");
        msgbox->Create(MSGT_TEXT, NULL, 0.5f, "Please wait...");
        GuiEffectFade gEF1(500, 0, true, 0.5f, false, 0.5f * GetWidth(), 1.0f * GetHeight());
        AddTop(msgbox, gEF1);
        // Init blueMSX emulator
        blueMsxInit(1);

        GuiEffectFade gEF2(50, 0, true, 0.5f, false, 0, 0);
        RemoveAndDelete(msgbox, gEF2);

        char *game_dir = NULL;
        GuiDlgDirSelect *dirs = new GuiDlgDirSelect(this, "dirselect", "Games", "dirlist.xml");
        dirs->SetVisible(false);
        AddTop(dirs);
        wii_log("guimain", "iterating dirlist.xml");
        for(;;) {
            Show(dirs, effectDefault);
            // Browse directory
            game_dir = dirs->Run();
            wii_log("guimain", "browse directory");
            wii_log_int("guimain", "game_dir var", (int) game_dir[0]);
            if( game_dir == NULL ) {
                RemoveAndDelete(dirs, effectDefault);
                dirs = NULL;
                break;
            }
            wii_log("guimain", "game menu");
            // Game menu
            ...
            for(;;) {
                ...
                prev = game;
                wii_log("guimain", "game selector");
                if( menu->Load(game_dir, "gamelist.xml", prev) ) {
                    wii_log("guimain", "gamelist.xml loaded");
                    ...
                    wii_log("guimain", "nogamelist popup");
		    GuiDlgMessageBox msgbox(this, "nogamelist");
                    msgbox.ShowPopup(this, "nogamelist",
                                     NULL, 0.5f, 2000,
                                     effectZoom, effectZoom,
        ...
        wii_log("guimain", "iterating dirlist.xml done");

#ifdef WII
    wii_log("guimain", "Main cleanup");
#endif
        // Destroy emulator
        emulatorExit();
        toolUnLoadAll();
    ...
    wii_log("guimain", "Main almost ending");
