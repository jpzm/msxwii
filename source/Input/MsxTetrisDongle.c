...
static UInt8 _read(MsxTetrisDongle* tetrisDongle) {
    return tetrisDongle->state;
}

static void _write(MsxTetrisDongle* tetrisDongle, UInt8 value) {
    if (value & 0x02) {
...
MsxJoystickDevice* msxTetrisDongleCreate()
{
    ...
    tetrisDongle->joyDevice.read      = _read;
    tetrisDongle->joyDevice.write     = _write;
